...
const HeaderLink: React.FC<{ item: HeaderItem }> = ({ item }) => {
  ...
  return (
    ...
    >
      <Link
        href={item.href}
        className={`text-xl flex font-medium duration-200  ${path === item.href ? "text-primary " : " text-black/50 dark:text-white/50 hover:text-primary dark:hover:text-primary"
          }`}
      >
        {item.label}
        {item.submenu && (
